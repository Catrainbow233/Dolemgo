package net.catrainbow.dolemgo.scheduler;
...
public class ServerScheduler {
    ...
    public void cancelTask(int taskId) {
        if (taskMap.containsKey(taskId)) {
            try {
                ...
                Server.getInstance().getLogger().error("Exception while invoking onCancel", ex);
            }
        }
    }
    ...
    private TaskHandler addTask(Plugin plugin, Runnable task, int delay, int period, boolean asynchronous) {
        if (plugin != null && !plugin.isEnabled()) {
            Server.getInstance().getLogger().error("Plugin '" + plugin.getName() + "' attempted to register a task while disabled.");
            return null;
        }
        if (delay < 0 || period < 0) {
            Server.getInstance().getLogger().error("Attempted to register a task with negative delay or period.");
            return null;
        }
        ...
        while (isReady(currentTick)) {
            ...
            if (taskHandler.isCancelled()) {
                ...
                try {
                    ...
                    Server.getInstance().getLogger().error("Could not execute taskHandler " + taskHandler.getTaskName() + ": " + e.getMessage());
